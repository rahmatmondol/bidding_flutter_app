...
class CustomerWorkPeopleDetailsController extends GetxController {
  ...
  Rx<GetBettingListDetailsModel> getBettingListDetailsModel = GetBettingListDetailsModel().obs;
  ...
  getCustomerBetListDetails(int id,int betingId)async{
    ...
    await BaseClient.safeApiCall(
      ...
      queryParameters: {
        "betting_list": "$betingId"
      },

      onSuccess: (response) {
        if (response.statusCode==200) {
          getBettingListDetailsModel.value= GetBettingListDetailsModel.fromJson(response.data);
